...
# +-------------------------------------------------------------------
# +-------------------------------------------------------------------
namespace sower\driver\model\relation;
use Closure;
use sower\App;
use sower\Collection;
use sower\service\database\Query;
use sower\service\database\Raw;
use sower\Exception;
use sower\Model;
use sower\driver\model\Pivot;
use sower\driver\model\Relation;
use sower\Paginator;
class BelongsToMany extends Relation
{
    ...
    {
        $class = $this->pivotName ?: '\\sower\\driver\\model\\Pivot';
        $pivot = new $class($data, $this->parent, $this->middle);

        if ($pivot instanceof Pivot) {
            return $pivot;
        } else {
            throw new Exception('pivot model must extends: \sower\driver\model\Pivot');
        }
    }
    ...
    public function getRelation(array $subRelation = [], Closure $closure = null): Collection
    {
        ...
        $result = $this->buildQuery()->relation($subRelation)->dataset();
     ...
     * @return Collection
     */
    public function dataset($data = null): Collection
    {
        $result = $this->buildQuery()->dataset($data);
        $this->hydratePivot($result);
    ...
    {
        $result = $this->buildQuery()->first($data);
    ...
    {
        return $this->buildQuery()->failException(true)->dataset($data);
    }
    ...
    {
        return $this->buildQuery()->failException(true)->first($data);
    }
